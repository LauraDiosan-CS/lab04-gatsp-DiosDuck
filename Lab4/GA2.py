...
class GA2:
    def __init__(self,popSize,vec,n):
    ...
    def __fitness(self,vec):
        ...
        for i in range(0,self.__n-1):
            fitness+=self.__distance(self.__vec[vec[i]],self.__vec[vec[i+1]])
        fitness += self.__distance(self.__vec[vec[-1]],self.__vec[vec[0]])
        return fitness

    def __distance(self,tup1,tup2):
        return math.sqrt((tup1[1]-tup2[1])*(tup1[1]-tup2[1])+(tup1[2]-tup2[2])*(tup1[2]-tup2[2]))